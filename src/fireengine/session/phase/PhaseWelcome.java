...
public class PhaseWelcome implements PhaseInterface {
	...
	private void sendGreeting() {
		this.sess
				.send(new ClientConnectionOutput("Welcome to Cyber World!", Client_IO_Colour.COLOURS.BRIGHTCYAN, null));
	}
	...
	private void menuDisplay() {
		...
		menuMsg.addPart("This is a placeholder menu.", null, null);
		menuMsg.newLine();
		// menuMsg.addPart("Please select from the following options(enter number):",
		// null, null);
		// menuMsg.newLine();
	...
	private void menuAction(Menu_Command command) {
		if (command != null) {
			...
			// sess.send(new ClientConnectionOutput("You have chosen to Create
			// Character."));
			// sess.send(new ClientConnectionOutput());
			...
			// sess.send(new ClientConnectionOutput("You have chosen to display the Menu
			// again."));
			// sess.send(new ClientConnectionOutput());
			// menuDisplay();
			// } else if (command instanceof Command_5) {
			// sess.send(new ClientConnectionOutput("You have chosen to display all player
			// info."));
			//
			...
			// sess.send(new ClientConnectionOutput("capatalise output: '" +
			// StringUtils.capitalise(input1) + "'"));
			// sess.send(new ClientConnectionOutput("sentence output: '" +
			// StringUtils.sentance(input1) + "'"));
			// sess.send(new ClientConnectionOutput());
			// String input2 = "Word";
			// sess.send(new ClientConnectionOutput("Input2: '" + input2 + "'"));
			// sess.send(new ClientConnectionOutput("capatalise output: '" +
			// StringUtils.capitalise(input2) + "'"));
			// sess.send(new ClientConnectionOutput("sentence output: '" +
			// StringUtils.sentance(input2) + "'"));
			// sess.send(new ClientConnectionOutput());
			// String input3 = "some sentance blah blah";
			// sess.send(new ClientConnectionOutput("Input3: '" + input3 + "'"));
			// sess.send(new ClientConnectionOutput("capatalise output: '" +
			// StringUtils.capitalise(input3) + "'"));
			// sess.send(new ClientConnectionOutput("sentence output: '" +
			// StringUtils.sentance(input3) + "'"));
			// sess.send(new ClientConnectionOutput());
			// String input4 = "Some sentance blah blah.";
			// sess.send(new ClientConnectionOutput("Input4: '" + input4 + "'"));
			// sess.send(new ClientConnectionOutput("capatalise output: '" +
			// StringUtils.capitalise(input4) + "'"));
			// sess.send(new ClientConnectionOutput("sentence output: '" +
			// StringUtils.sentance(input4) + "'"));
			// sess.send(new ClientConnectionOutput());
			// }
		} else {
