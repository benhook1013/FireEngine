...
public class CreateExit extends PlayerAction {
	private static Pattern pattern = compilePattern("CREATE EXITS? (\\w+)");
		...
		if (character instanceof Player) {
			if (Character.checkMapEditorPrivs((Player) character) == false) {
				MyLogger.log(Level.WARNING,
						"CreateExit: Non-map editor Character tried to doAction on a map editor only action that should have already been caught in the Action callAction checks.");
				return output;
			}

		} else {
			MyLogger.log(Level.WARNING,
					"CreateExit: Non-player Character tried to doAction on a Player-only action that should have already been caught in the Action callAction checks.");
			return output;
		}
	...
	public ClientConnectionOutput doAction(Character character, Direction.DIRECTION direction) {
		...
		try {
			...
			output.addPart(String.format("RoomExit created %s from \"%s\".", direction.toString(),
					character.getRoom().getName()), null, null);
			return output;
		} catch (MapExceptionRoomNull e) {
			output.addPart("Can't create exit as player room is null.", null, null);
			return output;
		} catch (MapExceptionExitRoomNull e) {
			output.addPart("Can't create exit as adjacent room is null.", null, null);
			return output;
			...
			MyLogger.log(Level.WARNING, "CreateExit: MapExceptionDirectionNotSupported while trying to createExit.", e);
			output.addPart(String.format(
